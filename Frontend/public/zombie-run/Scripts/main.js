...
function getGameApiBaseUrl() {
	...
	var queryValue = params.get('apiBaseUrl');
	var storedValue = window.localStorage.getItem('gameApiBaseUrl');
	var fallbackValue = window.location.protocol + '//' + window.location.hostname + ':5000';
...
function submitZombieRunScore(score) {
	var normalizedScore = Math.max(0, Math.floor(Number(score) || 0));

	if(!normalizedScore)
		return Promise.resolve(null);

	return fetch(getGameApiBaseUrl() + '/api/game/tasks/submit-game-score', {
		method: 'POST',
		credentials: 'include',
		headers: {
		...
		return response.json().catch(function() {
			...
			if(!response.ok || !data.success)
				throw new Error(data.message || 'Failed to sync Zombie Run score');
...
var authLockOverlay = null;
var gameOverSummary = null;
var gameOverCoins = null;
var gameOverWalletCoins = null;
var playAgainButton = null;
var activeLevel = null;

function hideZombieRunGameOver() {
	if(gameOverOverlay)
		gameOverOverlay.addClass('hidden');
}

function showZombieRunAuthLock() {
	if(authLockOverlay)
		authLockOverlay.removeClass('hidden');
}

function hideZombieRunAuthLock() {
	if(authLockOverlay)
		authLockOverlay.addClass('hidden');
}

function requireZombieRunSignIn() {
	return fetch(getGameApiBaseUrl() + '/api/game/auth/me', {
		credentials: 'include'
	}).then(function(response) {
		return response.json().catch(function() {
			return {};
...
function showZombieRunGameOver(runCoins, awardedCoins) {
	if(!gameOverOverlay)
		return;

	gameOverCoins.text(String(runCoins || 0));
	gameOverWalletCoins.text(String(awardedCoins || 0));
	gameOverSummary.text('Every run coin now becomes one wallet coin in the game app.');
	gameOverOverlay.removeClass('hidden');
}

function restartZombieRunGame() {
	if(!activeLevel)
		return;

	hideZombieRunGameOver();
	activeLevel.reset();
	activeLevel.load(definedLevels[0]);
	activeLevel.start();
}
...
var Level = Base.extend({
	...
	reload: function() {
		...
		if(settings.lifes < 0) {
			this.gameOver = true;
			submitZombieRunScore(settings.coins || 0)
				.then(function(data) {
					showZombieRunGameOver(settings.coins || 0, data && data.awardedCoins ? data.awardedCoins : 0);
				})
				.catch(function(error) {
					if(window.console && typeof window.console.error === 'function')
						window.console.error(error);

					showZombieRunGameOver(settings.coins || 0, 0);
				});
			return;
		} else {		
			this.load(this.raw);
...
$(document).ready(function() {
	...
	activeLevel = level;
	authLockOverlay = $('#authLockOverlay');
	gameOverOverlay = $('#gameOverOverlay');
	gameOverSummary = $('#gameOverSummary');
	gameOverCoins = $('#gameOverCoins');
	gameOverWalletCoins = $('#gameOverWalletCoins');
	playAgainButton = $('#playAgainButton');
	playAgainButton.on('click', restartZombieRunGame);
	hideZombieRunGameOver();
	hideZombieRunAuthLock();
	requireZombieRunSignIn().then(function(isAuthenticated) {
		if(!isAuthenticated) {
			showZombieRunAuthLock();
			return;
		}
